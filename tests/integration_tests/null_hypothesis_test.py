...
def phantom_disk():
    dgm = _load_or_compute_dgm("disk",   _make_disk,   _ripser_h1)
    return Phantom(dgm, k=1)


@pytest.fixture(scope="module")
def phantom_circle():
    dgm = _load_or_compute_dgm("circle",   _make_circle,   _ripser_h1)
    return Phantom(dgm, k=1)


@pytest.fixture(scope="module")
def phantom_torus():
    dgm = _load_or_compute_dgm("torus",   _make_torus,   _ripser_h1)
    return Phantom(dgm, k=1)
...
class TestHypothesisTestReturnStructure:
    ...
    def test_result_is_ndarray(self, phantom_circle):
        result = phantom_circle.hypothesis_test()
        assert isinstance(result["universal_null"]
                          ["results_array"], np.ndarray)

    def test_result_has_five_columns(self, phantom_circle):
        result = phantom_circle.hypothesis_test()
        arr = result["universal_null"]["results_array"]
...
class TestHypothesisTestGeometry:
    ...
    def test_stricter_alpha_does_not_increase_rejections(self, phantom_circle):
        ...
        n_loose = result_loose["universal_null"]["results_array"][:, 4].sum()
        n_strict = result_strict["universal_null"]["results_array"][:, 4].sum()
